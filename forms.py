...
previews = ["partial", "full", "free-ebooks", "paid-ebooks", "ebooks"]
orderBy = ["newest", "relevance"]

class BookConditionsForm(FlaskForm):

    general_search = StringField(
        "Enter Title, Author, ISBN or a combination of the 3", validators=[InputRequired()])
    order_by_filter = SelectField("Order By", choices=[(order, order) for order in orderBy])
    preview_filter = SelectField("Preview availablity", choices = [(preview, preview) for preview in previews])
